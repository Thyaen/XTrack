...
def generate_launch_description():
    ...
    slam_config_path_localization = os.path.join(get_package_share_directory('pcps_ros'), 'config', 'slam_localization.yaml')
    ...
    slam = IncludeLaunchDescription(
            ...
            launch_arguments={
                'slam_params_file': slam_config_path_localization,
                'use_sim_time': 'false',
            }.items()
    )

    nav2_path = get_package_share_directory('nav2_bringup')
    nav2_config_path = os.path.join(get_package_share_directory('pcps_ros'), 'config', 'nav2_params.yaml')
    ...
    ip_address = LaunchConfiguration("ip_address", default="192.168.0.111")

    server_speaker = Node(
        package='pcps_ros',
        executable='status_server.py',
        output='screen',
